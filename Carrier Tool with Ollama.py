...
#llm = LLM(model="groq/llama-3.3-70b-versatile", api_key=groq_api_key, temperature=0.7)

llm = LLM(
    model="ollama/llama3",  # or "ollama/llama3"
...
if uploaded_file:
    ...
    search_jobs_task = Task(
        description=(
            ...
            "Return a Markdown list of the top 2-3 valid job postings, each with:\n"
            "  - **Title**\n"
    ...
    crew = Crew(
        tasks=[cv_question_task,extraction_task,suggestion_task,search_jobs_task],
        agents=[cv_analysis_agent, skill_agent, improvement_agent,job_search_agent],
